...
public class HeaderScrollHelper {
    ...
    public static boolean isRecyclerViewTopCompleteVisiable(RecyclerView recyclerView) {
        if (recyclerView != null) {
            RecyclerView.LayoutManager layoutManager = recyclerView.getLayoutManager();
            if (layoutManager instanceof LinearLayoutManager) {
                ...
                int[] mFirstVisibleItems = null;
                int[] firstVisibleItemPositions = ((StaggeredGridLayoutManager) layoutManager).findFirstCompletelyVisibleItemPositions(mFirstVisibleItems);
                View childAt = recyclerView.getChildAt(0);
                if (childAt == null || firstVisibleItemPositions[0] == 0 && childAt.getTop() == 0) {
                    return true;
                }
    ...
    public static boolean isRecyclerViewBottomCompleteVisiable(RecyclerView recyclerView) {
        if (recyclerView != null) {
            RecyclerView.LayoutManager layoutManager = recyclerView.getLayoutManager();
            if (layoutManager instanceof LinearLayoutManager) {
                int LastItemPosition = ((LinearLayoutManager) layoutManager).findLastCompletelyVisibleItemPosition();
                int visibleItemCount = layoutManager.getChildCount();
                int totalItemCount = layoutManager.getItemCount();
                if (LastItemPosition == totalItemCount - 1 && visibleItemCount > 0) {
                    return true;
                }
            } else if (layoutManager instanceof StaggeredGridLayoutManager) {
                int[] mFirstVisibleItems = null;
                int[] firstLastItemPositions = ((StaggeredGridLayoutManager) layoutManager).findLastCompletelyVisibleItemPositions(mFirstVisibleItems);
                int visibleItemCount = layoutManager.getChildCount();
                int totalItemCount = layoutManager.getItemCount();
                if (firstLastItemPositions[0] == totalItemCount - 1 && visibleItemCount > 0) {
                    return true;
