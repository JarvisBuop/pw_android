...
import com.example.libimagefilter.filter.advanced.*
import com.example.libimagefilter.filter.base.gpuimage.*
import com.example.libimagefilter.filter.origin.*
...
class FilterAdjuster(filter: GPUImageFilter?) {
    ...
    init {
        adjuster = when (filter) {
            ...
            is MagicImageAdjustFilter -> MagicImageAdjuster(filter)
    ...
    private inner class MagicImageAdjuster(filter: MagicImageAdjustFilter) :
            Adjuster<MagicImageAdjustFilter>(filter) {
        override fun adjust(percentage: Int) {
            filter.setBrightness(range(percentage, -1.0f, 1.0f))
            filter.setContrast(range(percentage, 0.0f, 2.0f))
            filter.setExposure(range(percentage, -10.0f, 10.0f))
