        ...
        public boolean onTouch(View v, MotionEvent event) {
            if (v instanceof ScrollView) {
                ...
                if ((sv.getScrollY() == (sv.getChildAt(0).getMeasuredHeight() - sv
                        .getMeasuredHeight()) || sv.getChildAt(0).getMeasuredHeight() < sv
                        .getMeasuredHeight()) && mCurrentViewIndex == 0 && !disableDoubleScroll)
                    canPullUp = true;
                else
                    canPullUp = false;
            } else if (v instanceof RecyclerView) {
                RecyclerView rlist = (RecyclerView) v;
                if (isScrollBottom(rlist)) {
                    canPullUp = true;
                } else {
                    canPullUp = false;
                }
...
//            firstCompletelyVisibleItemPosition = ((GridLayoutManager) layoutManager).findLastCompletelyVisibleItemPosition();
    ...
    private boolean disableDoubleScroll;

    public void setDisableDoubleScroll(boolean disableDoubleScroll) {
