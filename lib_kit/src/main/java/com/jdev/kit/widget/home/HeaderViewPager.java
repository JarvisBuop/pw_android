...
public class HeaderViewPager extends LinearLayout {
    ...
    protected int topOffset = 0;      //滚动的最大偏移量

    protected Scroller mScroller;
    protected int mTouchSlop;         //表示滑动的时候，手的移动要大于这个距离才开始移动控件。
    protected int mMinimumVelocity;   //允许执行一个fling手势动作的最小速度值
    protected int mMaximumVelocity;   //允许执行一个fling手势动作的最大速度值
    protected int sysVersion;         //当前sdk版本，用于判断api版本
    protected View mHeadView;         //需要被滑出的头部
    protected int mHeadHeight;        //滑出头部的高度
    protected int maxY = 0;           //最大滑出的距离，等于 mHeadHeight
    protected int minY = 0;           //最小的距离， 头部在最顶部
    protected int mCurY;              //当前已经滚动的距离
    protected VelocityTracker mVelocityTracker;
    protected int mDirection;
    protected int mLastScrollerY;
    protected boolean mDisallowIntercept;  //是否允许拦截事件
    protected boolean isClickHead;         //当前点击区域是否在头部
    protected OnScrollListener onScrollListener;   //滚动的监听
    protected HeaderScrollHelper mScrollable;
    ...
    public void setCurrentScrollableContainer(HeaderScrollHelper.ScrollableContainer scrollableContainer) {
        if (scrollableContainer == null || scrollableContainer.getScrollableView() == null) {
            scrollableContainer = new HeaderScrollHelper.ScrollableContainer() {
                @Override
