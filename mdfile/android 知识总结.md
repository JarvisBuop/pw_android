...
逻辑结构 是抽象的概念,依赖于物理结构; 物理结构就是数组和链表等基础的数据结构;

> 常见数据结构

- stack 栈: FILO; 

出/入栈 时间复杂度都为 O(1); 最早进入的元素位置为栈底,最后进入的元素位置为栈顶;

栈的应用:  回溯历史; 面包屑导航;

- queue 队列: FIFO; 

队列出口端为对头front,队列入口端为对尾rear;  对尾的位置一般为最后入队元素的下一个位置; 出/入队 时间复杂度为 O(1);

用数组实现的队列可以采用 `循环队列`的方式类维持队列容量的恒定(一直到`(对尾下标+1)%数组长度 = 对头下标`表示队列已满,可扩容,队列的实际容量比数组长度小1);

队列的应用: 回放历史; 多线程顺序;

双端队列deque: 两端都可以入队和出队;

优先队列: 谁的优先级高谁先出队(二叉堆);

- 散列表

哈希表,key-value; 查找时间复杂度 O(1);

哈希函数: 作用是将Key和数组下标进行转换; 最简单的转化方式就是按照数组长度进行取模运算; `index = HashCode(Key) % Array.Length`; 获取到指定下标,从而获取到value;

哈希冲突: Key转化为下标的位置一样,产生hash冲突;

使用`开放寻址法`,`链表法`解决hash冲突;

开放寻址法: 当一个key通过哈希函数获得对应的数组下标已被占用时,寻找下一个空档位置; `ThreadLocal`使用的就是开放寻址法;

链表法: 数组的每一个元素不仅是一个Entry对象,还是一个链表的头节点;

扩容(resize) 当多次元素插入,key映射位置发生冲突的概率提高,形成很长的列表会影响效率;

扩容的过程:

1. 扩容,创建一个新的entry空数组,长度是原数组的两倍;
2. 重新hash,遍历原数组,把所有的entry重新hash到新数组中;

### 树和二叉树

树是n(n>=0)个节点的有限集; n=0为空树; 
...
满二叉树: 所有的非叶子节点都存在左右孩子,且所有叶子节点都在同一层级上;

完全二叉树: 对一个有n个节点的二叉树,按层级顺序编号,则所有节点的编号为从1到n; 如果这个树所有节点和同样深度的满二叉树的编号为从1到n的节点位置相同,则这个二叉树为完全二叉树;

![在这里插入图片描述](https://img-blog.csdnimg.cn/20200918105430141.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L01ySmFydmlzRG9uZw==,size_16,color_FFFFFF,t_70#pic_center)

二叉树的物理结构: 1.链式存储结构; 2.数组;

使用数组存储时,会按照层级顺序把二叉树的节点放在数组中对应的位置上; 某个左右孩子缺失,数组相应位置可空出来; 可以快速的定位到二叉树的孩子节点和父节点; 父节点坐标为x,左孩子下标2*x+1,右孩子下标2*x+2;

##### 二叉树的应用

主要用于 `查找操作和维持相对顺序`这两个方面;
...
- 左子树的所有节点的值都小于根节点的值;
- 右子树上所有节点的值都大于根节点的值;  和二分查找法比较类似;

时间复杂度: O(logn) 和树的深度是一样的;

2.`二叉排序树`(也是二叉查找树) - 维持相对顺序

二叉树的`自平衡` 为了解决单个子树插入过多退化成链表,查询的时间复杂度退出成O(n); 如红黑树,AVL树,树堆等;
...
非线性数据的遍历, 二叉树的遍历归结为`深度优先遍历`,`广度优先遍历`;
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20200918141313272.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L01ySmFydmlzRG9uZw==,size_16,color_FFFFFF,t_70#pic_center)
...
本质上是一个`完全二叉树`,分为 最大堆,最小堆; 二叉堆的根节点叫做 `堆顶`;

`最大堆`: 任何一个节点的值,都大于或等于它左右孩子节点的值; 最大堆的堆顶是整个堆中的最大元素;

`最小堆`: 任何一个节点的值,都小于或等于它左右孩子节点的值; 最小堆的堆顶是整个堆中的最小元素;

二叉堆也维持着相对位置,不过相对于二叉查找树条件宽松一点,只要求父节点比他的孩子节点大;

二叉堆的时间复杂度: 插入 (O(logn)),删除(O(logn))

构建二叉堆 (O(n))

##### 二叉堆的自我调整;

`插入节点,删除节点,构建二叉树`; 都是基于堆的自我调整,就是将一个不符合堆性质的完全二叉树,调整成一个堆;

插入节点: 新节点跟父节点比较;

删除节点: 最小堆的情况,与子节点的最小值比较;

构建二叉堆: 本质上是让所有的非叶子节点依次下沉(最小堆,使每一个孩子节点都大于根节点);

### 优先队列

二叉堆是实现优先队列和堆排序的基础;

- `最大优先队列`,无论入队顺序如何,都是当前最大的元素优先出队;`最小优先队列`类似;
- 因为二叉堆,最大堆的堆顶是整个堆的最大元素,最大堆的每次入队操作就是堆的插入操作,上浮至合适位置; 每一次出队操作就是删除堆顶节点,将最后一个元素放入堆顶,下沉到合适位置;

时间复杂度为 插入(O(lgn)) 删除(O(lgn))
...
- 冒泡排序 
	- 鸡尾酒排序(相对于冒泡排序,增加了元素比较和交换过程是双向的;),一次循环为先从左到右再从右到左; 即先确定两边的有序数组;可应用在大部分元素已经有序的情况;
- 选择排序
- 插入排序
- 希尔排序(略优于O(n^2))
...
- 快速排序 
	- `分治法`: 每一轮挑选一个基准元素,让其他比它大的元素移动到数列一边,比它小的元素移动到数列另一边,将数列拆解成两个部分;
	- 元素个数为n,平均情况下需要logn轮,因此快排的时间复杂度为O(nlogn);最坏情况下O(n^2)
	- 基准元素的选择 pivot,以基准元素为中心,把其他元素移动至它的两端; 
		- 可选择基准元素为第一个元素,但是如果完全是逆序的,退化成O(n^2);
		- 可随机选择一个元素作为基准元素,且让基准元素和数列首元素交换位置;
	- 元素的交换
		- 双边循环法 (数组两边交替遍历元素)
			- 选定基准元素pivot,设置left和right指针;
			- 先比较right指针和pivot的值,找到pivot小的值;切换至left指针,比较left指针和pivot的值,找到大于pivot的值,停止比较,交换left和right指针指向的值,left <right 重复操作;
			- 最后全部交换完成,left==right,交换pivot放于两部分中间;完成第一次循环;
		- 单边循环法 (数组的一边进行遍历和交换)
			- 选定基准元素pivot,设置mark指针指向数列起始位置,这个mark指针代表小于基准元素的区域边界;
			- 从下一个元素开始遍历,如果小于基准元素,将mark指针右移,且交换当前指向元素和mark元素,小于pivot的区域变大;
			- 将mark和小于的index的位置交换;完成第一次循环;
- 归并排序
- 堆排序 (二叉堆中的最大堆和最小堆)
	- 原理
		- 把无序数组构成二叉堆;需要从小到大排序,则构成最大堆;需要从大到小排序,则构成最小堆;
		- 循环删除堆顶元素,并将该元素移到集合尾部,调整堆产生新的堆顶;
	- 时间复杂度 为 O(nLogn),空间复杂度 O(1); 因为第一步构建二叉堆的时间复杂度为O(n),第二步的计算规模是(n-1)logn,时间复杂度为O(nlogn)
	- 堆排序和快排平均复杂度都是O(nlogn),都是不稳定排序; 不同点快排最坏情况复杂度为o(n^2),堆排序最坏情况稳定在O(nlogn); 快排的空间复杂度为O(logn),堆排序的空间复杂度为O(1);
...
- 计数排序
	- 原理:
		- 获取数列的最大值
		- 根据最大值决定数组的长度,遍历和统计数据
		- 遍历统计数组,输出结果;
	- 优化后的原理: 稳定排序
		- 使用最大值-最小值确定新建数组范围
		- 遍历统计数组;
		- 从统计数组的第2个元素开始，每一个元素都加上 前面所有元素之和; 为了让统计数组存储的元素值,等于相应整数的最终排序位置的序号(元素值-最小值-1的索引即排序索引,因为统计数组中已经有序了);
		- 倒序遍历原始数据,从统计数组找到正确位置,输出到结果数组;
	- 时间复杂度为 3n+m,O(n+m); 空间复杂度 为 O(m);
	- 缺点: 
		- 当数列的最大和最小值差距过大时,不适合使用计数排序;
		- 数列元素不是整数时,不适合使用计数排序;
- 桶排序 bucket 代表一个区间范围,可承载一个或多个元素;
	- 创建桶,确定每一个桶的区间范围;
		- 最后一个桶只包含数列的最大值,其他桶的区间按照比例来确定; `区间跨度 = (最大值-最小值)/(桶的数量-1)`
	- 遍历原始数列,将元素放入桶中;
	- 对每个桶内部的元素分别进行排序;
	- 遍历所有的桶,输出所有元素;
